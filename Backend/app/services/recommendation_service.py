...
class RecommendationService:
    ...
    def _calculate_tfidf_similarity(
        ...
        try:
            ...
            tfidf_scores = {item_ids[i]: float(similarities[i]) for i in range(len(item_ids))}
    ...
    def get_recommended_jobs(
        ...
        if user:
            ...
            def job_text_func(job):
                return " ".join([
                    ...
                    job.recommended_experience_level.value if job.recommended_experience_level else ""
                ])
            
            tfidf_scores = self._calculate_tfidf_similarity(user_text, jobs, job_text_func)
        ...
        for job, vector_score in vector_results:
            ...
            if tfidf_scores:
                combined_score = (embedding_weight * float(vector_score)) + (tfidf_weight * tfidf_score)
            else:
        ...
        logger.info(f"Found {len(final_results)} recommended jobs (hybrid TF-IDF + vector)")
    ...
    def get_recommended_resources(
        ...
        vector_results = (
            db.query(
                Resource,
                (1 - Resource.embedding.cosine_distance(user_embedding)).label("similarity")
            )
        ...
        if user:
            ...
            tfidf_scores = self._calculate_tfidf_similarity(user_text, resources, resource_text_func)
        ...
        logger.info(f"Found {len(final_results)} recommended resources (hybrid TF-IDF + vector)")
        return final_results
    ...
    def get_similar_resources(
        ...
        results = (
            db.query(
                Resource,
                (1 - Resource.embedding.cosine_distance(resource_embedding)).label("similarity")
            )
            ...
            .order_by((1 - Resource.embedding.cosine_distance(resource_embedding)).desc())
            .limit(limit)
            .all()
        )
        
        logger.info(f"Found {len(results)} similar resources for resource {exclude_resource_id}")
