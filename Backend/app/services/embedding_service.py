...
class EmbeddingService:
    ...
    def __init__(self):
        ...
        if self._model is None:
            try:
                ...
                logger.critical(f"Failed to initialize FastEmbed model: {e}", exc_info=True)
                raise EmbeddingGenerationError(f"Failed to initialize FastEmbed model: {e}")
    ...
    def generate_job_embedding(self, job: Job) -> List[float]:
        ...
        experience_text = job.recommended_experience_level.value if job.recommended_experience_level else ""
